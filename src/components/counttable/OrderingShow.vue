<template>
  ...
  <body id="poster">
    <el-col :xs="24" :sm="20" :md="16" :lg="12" :xl="8">
      <!-- 权限在此处判断 -->
      <el-container style="" v-if="this.ruleForm.user_name== ''">
        ...
        <el-main style="text-align:center">
          <el-form :model="ruleForm" :rules="rules" border ref="ruleForm" label-width="100px" class="demo-ruleForm"
            ...
            <el-form-item label="就餐类型" prop="type">
            ...
            <el-form-item label="就餐人姓名" prop="user_name" style=";text-align:left;width:250px">
              <el-input v-model="ruleForm.user_name" :disabled="true"></el-input>
            </el-form-item>
            <!-- <el-form-item label="备注" prop="remarks" style="width:300px;text-align:left">
...
export default {
  ...
  data () {
    return {
      ...
      ruleForm: {
        user_name: '',
        eat_date: (new Date).getFullYear() + "-" + ((new Date).getMonth() + 1) + "-" + ((new Date).getDate()),
        type: [],
        eat_type: '',
        remarks: ''
      },
  ...
  methods: {
    ...
    handleEdit (index, row) {
      ...
      //编辑对用户名进行赋值.
      this.$axios.post('/v1/SelectOrderingById', { id: row.id }).then(resp => {
    ...
    //获取订餐信息
    getOrderingInfo () {
      this.$axios.post('/v1/SelectOrdering', { type: this.value_type, user: '', eat_time: this.value_date }).then(resp => {
        if (resp && resp.status == 200) {
    ...
    insertdata () {
      ...
      console.log(this.ruleForm.eat_type);
      this.$axios
